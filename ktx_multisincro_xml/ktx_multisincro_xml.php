 ...
 * formato XML.  Genera CRON para automatizar la tarea en CRON manager
 * del servidor
 * 
 */
...
class Ktx_MultiSincro_Xml extends Module
{

    public function __construct()
    {
        $this->name = 'ktx_multisincro_xml';
        $this->version = '1.0.0';
        $this->author = 'Katuxos';
        $this->bootstrap = true;
        ...
        $this->description = $this->l('Configuración para actualizar Stock y Precio con tarea CRON. (XML)');
    ...
    public function install()
    {
        if (!parent::install() || !(Db::getInstance()->Execute('CREATE TABLE IF NOT EXISTS `'. _DB_PREFIX_ .
                'ktx_multisincro_xml` (`id` INT NOT NULL AUTO_INCREMENT, `id_proveedor` INT(11) DEFAULT 0, '
                ...
                . '`otros` VARCHAR(255) DEFAULT NULL, `xml` VARCHAR(255) DEFAULT NULL, '
                . '`usuario` VARCHAR(255) DEFAULT NULL, `password` VARCHAR(255) DEFAULT NULL, '
    ...
    {
        if (Tools::isSubmit('addktx_multisincro_xml')) {  /* Crear nueva configuración de archivo XML para un proveedor */
            return $this->generaFormulario();
        } elseif (Tools::isSubmit('saveKtx_Multisincro_xml')) {
            /* Salvar nueva configuración de archivo XML para un proveedor */
            $katuxos = new katuxosXmlClass();
            ...
            $katuxos->setOtros((string)Tools::getValue('otros', '0'));
            $katuxos->setCosto_Adicional((float)Tools::getValue('costo_adicional', 0));
            $katuxos->setMargen((float)Tools::getValue('margen', 0));
            $katuxos->setXml((string)Tools::getValue('xml', 'Error'));
            $katuxos->setUsuario((string)Tools::getValue('usuario', '0'));
            $katuxos->setPassword((string)Tools::getValue('password', '0'));
            if ($katuxos->save()) {
        ...
        } elseif (Tools::isSubmit('viewktx_multisincro_xml')) {  /* Hacer cambios a un registro existente */
            return $this->formularioEdicion();
        } elseif (Tools::isSubmit('saveEditKtx_Multisincro_xml')) {  /* Salvar cambios a un registro existente */
            ...
            Tools::redirectAdmin(AdminController::$currentIndex.'&configure='.$this->name.'&token='
                    .Tools::getAdminTokenLite('AdminModules'));
        } elseif (Tools::isSubmit('deletektx_multisincro_xml')) {
            /* Eliminar configuración de archivo XML para un proveedor */
            $katuxos = new katuxosXmlClass((int)Tools::getValue('id_xml'));
            if ($katuxos->delete()) {
    ...
    {
        $resultados = Db::getInstance()->ExecuteS('SELECT kc.id as id_xml, sp.name as nombre_proveedor, '
                . 'kc.cron as tareacron FROM `'._DB_PREFIX_.'ktx_multisincro_xml` kc JOIN `'._DB_PREFIX_.
        ...
        $helper->module = $this;
        $helper->title = $this->l('Proveedores con Actualización mediante XML');
        $helper->_pagination = array(10, 20, 50, 100, 200);
        $helper->table = _DB_PREFIX_.$this->name.'_xml';
    ...
    public function generaFormulario()
    {
        ...
        $this->fields_form = array (
            'form' => array(
                'legend' => array(
                    'title' => $this->l('Nueva sincronización XML'),
                ),
                'input' => array(
                    'id_proveedor' => array(
                        'type' => 'select',
                    ...
                    'ruta' => array(
                        'type' => 'text',
                        'label' => $this->l('Ruta XML:'),
                        'desc' => $this->l('Ruta XML (ejemplo: http://webdetuproveedor.com/xml.htm)'),
                        'name' => 'ruta',
                        'required' => true,
                    ),
                    'referencia' => array(
                        ...
                        'desc' => $this->l('Nombre del campo que tiene la referencia del producto'),
                        'name' => 'referencia',
                        'required' => true,
                    ),
                    'stock' => array(
                        'type' => 'text',
                        'label' => $this->l('Stock:'),
                        'desc' => $this->l('Nombre del campo que tiene el stock. Si no deseas actualizar '
                                . 'el stock, deja en 0.'),
                        'name' => 'stock',
                    ),
                    'precio' => array(
                        ...
                        'desc' => $this->l('Nombre del campo que tiene el precio base del proveedor '
                    ...
                    'canon' => array(
                        'type' => 'text',
                        'label' => $this->l('Canon:'),
                        'desc' => $this->l('Nombre del campo que tiene el canon a añadir al precio del producto.'
                                . 'Si no hay canon, deja en 0.'),
                        'name' => 'canon',
                    ),
                    'otros' => array(
                        'type' => 'text',
                        'label' => $this->l('Otros Costos Extras:'),
                        'desc' => $this->l('Nombre del campo que tiene costos extras a añadir al precio del producto.'
                                . 'Por ejemplo: envíos.  Si no hay, deja en 0.'),
                        'name' => 'otros',
                    ),   
                    ...
                    'margen' => array(
                        ...
                        'desc' => $this->l('Margen para cálculo de precio (ejemplo: 20, 5, etc.). El precio antes '
                                ...
                                . '(PRECIO DEL PROVEEDOR + Otros Costos + Canon + Extras) * (1+ (Margen/100))'),
                        'name' => 'margen',
                    ),
                    'xml' => array(
                        'type' => 'text',
                        'label' => $this->l('Nombre de Estructura XML:'),
                        'desc' => $this->l('Info Técnica. Pregunta a tu proveedor o informático.  Nombre del '
                                . 'array XML que viene después de diffgram->DocumentElement->.'),
                        'name' => 'xml',
                    ),
                    'usuario' => array(
                        'type' => 'text',
                        'label' => $this->l('Usuario:'),
                        'desc' => $this->l('Usuario para conexión XML.'),
                        'name' => 'usuario',
                    ),
                    'password' => array(
                        'type' => 'text',
                        'label' => $this->l('Password:'),
                        'desc' => $this->l('Password del usuario para la conexión XML.'),
                        'name' => 'password',
                    )                    
                ),
                'submit' => array(
        ...
        $helper->title = $this->l('Configurar nueva actualización por XML');
        $helper->submit_action = 'saveKtx_Multisincro_xml';
        $helper->table = _DB_PREFIX_.$this->name.'_xml';
        $helper->token = Tools::getAdminTokenLite('AdminModules');
        $helper->currentIndex = AdminController::$currentIndex.'&configure='.$this->name;
    ...
    public function formularioEdicion()
    {
            ...
            'form' => array(
                ...
                'input' => array(
                    ...
                    'stock' => array(
                        'type' => 'text',
                        'label' => $this->l('Stock:'),
                        'desc' => $this->l('Nombre del campo que tiene el stock. Para dejar de actualizar el '
                                . 'stock, escribe 0.'),
                        'name' => 'stock',
                    ),
                    'precio' => array(
        ...
        $helper = new HelperForm();
        $helper->title = $this->l('Actualizar configuración de actualización por XML');
        $helper->submit_action = 'saveEditKtx_Multisincro_xml';
        $helper->table = _DB_PREFIX_.$this->name.'_xml';
        $helper->token = Tools::getAdminTokenLite('AdminModules');
        $helper->currentIndex = AdminController::$currentIndex.'&configure='.$this->name;
        $helper->show_toolbar = true;
        ...
        $helper->tpl_vars = array(
            'fields_value' => array(
                ...
                'otros' => $katuxos->getOtros(),
                'costo_adicional' => $katuxos->getCosto_Adicional(),
                'margen' => $katuxos->getMargen(),
                'xml' => $katuxos->getXml(),
                'usuario' => $katuxos->getUsuario(),
                'password' => $katuxos->getPassword(),
    ...
    {
        $cron = _PS_BASE_URL_.__PS_BASE_URI__.'modules/ktx_multisincro_xml/ktx_cron_xml.php?token='.$codigo.'&id='.$id;
        return $cron;
    }
}
